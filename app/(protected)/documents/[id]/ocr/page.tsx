...
export default async function DocumentOcrPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const providerUsed = ((ocrResult?.rawJson as Record<string, unknown> | null)?.provider as string | undefined) ?? "unknown";
  return <div className="grid gap-4 md:grid-cols-2"><section className="rounded border p-3"><h3 className="mb-2 font-semibold">Document Preview</h3><p className="text-sm text-slate-600">File: {document.fileName}</p><p className="text-sm text-slate-600">Storage path: {document.storagePath}</p><p className="mt-3 rounded border border-dashed p-4 text-sm">Placeholder preview panel (mock rendering only).</p><form action={runAction} className="mt-3"><button type="submit" className="rounded bg-slate-900 px-3 py-2 text-white">Run OCR</button></form></section>
  <section className="rounded border p-3"><h3 className="mb-2 font-semibold">Editable OCR Result</h3><p className="mb-2 text-xs text-slate-500">OCR provider: {providerUsed}</p><form action={saveAction} className="space-y-2">{["invoiceNumber","invoiceSymbol","invoiceDate","sellerName","sellerTaxCode","sellerAddress","buyerName","buyerTaxCode","buyerAddress","subtotal","vatAmount","totalAmount","currency"].map((name)=><input key={name} name={name} defaultValue={fields?.[name] ?? ""} placeholder={name} className="w-full rounded border p-2" />)}<textarea name="lineItems" defaultValue={JSON.stringify(ocrResult?.lineItems.map((item)=>({ label: item.label, value: item.value, confidence: item.confidence })) ?? [], null, 2)} className="min-h-36 w-full rounded border p-2" /><button className="rounded border px-3 py-2" type="submit">Save OCR corrections</button></form><form action={submitAction} className="mt-2"><button className="rounded bg-slate-900 px-3 py-2 text-white" type="submit">Submit for approval</button></form></section></div>;
}
